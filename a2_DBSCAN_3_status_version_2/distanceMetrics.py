...
class dMetrics:
    ...
    def findMaxByCenter(self):
        ...
        if metric_type == 'euclidean':
            center_obj = np.mean(data, axis=0)
            center_d_max = np.max(np.sum((center_obj - data)**2, axis=1))
        elif metric_type == 'juravlov':
            center_obj = np.ones_like(data[0])
            for i in range(len(characteristic_vector)): # Markazni topish
                if characteristic_vector[i] == 1:
                    center_obj[i] = np.mean(data[:,i])
                else:
                    max_index = np.argmax(np.unique_all(data[:,i]).counts)
                    max_values = np.unique_all(data[:,i]).values[max_index]
                    center_obj[i] = max_values

            q_arr_1 = characteristic_vector == 1
            q_arr_0 = characteristic_vector == 0
            center_d_max = -1
            for i in range(m):      # Max masofani topish
                if np.any(q_arr_1 == True):
                    q_sum = np.max(np.abs(center_obj[q_arr_1] - data[i, q_arr_1]))
                n_sum = np.sum(center_obj[q_arr_0]!=data[i, q_arr_0])
                if center_d_max < (q_sum + n_sum):
                    center_d_max = q_sum + n_sum

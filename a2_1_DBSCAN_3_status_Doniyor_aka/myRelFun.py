...
class relatedFunction:
    ...
    def K1relatedFun(self):      # Ikkinchi sinf ideal holat uchun olinganda. K2 mavjud bo'lmagan holat uchun
        ...
        for j in range(cols):
            for u in range(1, 3):  # 1, 2, 3 - statuslar bo'lgani uchun
                indexes_u = np.where(data[:, j] == u)
                d_u = np.sum(data[:, j] == u)
                new_data[indexes_u, j] = np.round((d_u / rows) / (d_u / rows + 1.0/2.0), 5)
    ...
    def stabilityFeatures(self):  # Turg'unlik hisoblash. Maqoladagi (3) formula bo'yicha
